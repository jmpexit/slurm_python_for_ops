...
def main():
    for k, v in itertools.product(generate_hosts(ip_addresses), generate_commands(commands)):
            #zip(generate_hosts(ip_addresses), generate_commands(commands))):
        if v != 'rm -rf /':
